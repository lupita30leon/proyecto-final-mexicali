...
var nombresIndices = delitos.getIndexes().map(function (indice) {
  return indice.name;
});

if (
  nombresIndices.indexOf("idx_clasificacion_fecha_id") === -1 ||
  nombresIndices.indexOf("idx_lugar_fecha_id") === -1
) {
  throw new Error(
    "Ejecuta primero scripts/crear_indices.js."
  );
}
...
print("=== MEDICIÓN DESPUÉS DE CREAR ÍNDICES ===");
